...
pub struct ImageMetadata {
    ...
    pub source: String, // "a1111", "comfyui", "novelai", "unknown"
...
/// Parse PNG tEXt chunks from a file
    ...
    loop {
        ...
        if chunk_type == "tEXt" {
            ...
            if let Some(null_pos) = data.iter().position(|&b| b == 0) {
                ...
                if rest.len() > 2 {
                    let mut pos = 2;
                    // Find null separators for language tag and translated keyword
                    for _ in 0..2 {
...
fn parse_a1111_params(params_str: &str) -> HashMap<String, String> {
    ...
    // Split by ", " but respect key: value pairs
...
fn parse_comfyui_metadata(raw: &HashMap<String, String>) -> ImageMetadata {
    ...
    if let Some(prompt_str) = raw.get("prompt") {
        if let Ok(workflow) = serde_json::from_str::<serde_json::Value>(prompt_str) {
            // Walk through nodes to find prompt text, model, sampler settings
                            ...
                            match class_type {
                                "CLIPTextEncode" => {
                                    if let Some(text) = inputs.get("text").and_then(|v| v.as_str()) {
                                        // Heuristic: if negative is in the class_type or we haven't set prompt yet
    ...
    // Also check workflow for additional info
    if meta.prompt.is_empty() {
...
fn parse_novelai_metadata(raw: &HashMap<String, String>) -> ImageMetadata {
    ...
    // NovelAI stores metadata in "Description" (prompt) and "Comment" (JSON with params)
        ...
        if let Ok(params) = serde_json::from_str::<serde_json::Value>(comment) {
            ...
            if let Some(v4_uc) = params.get("v4_negative_prompt") {
                if let Some(caption) = v4_uc.get("caption") {
                    ...
                    // Also collect negative char_captions if present
                    if let Some(char_captions) = caption.get("char_captions").and_then(|v| v.as_array()) {
                        ...
                        if !neg_chars.is_empty() {
                            // Append negative character prompts to the negative_prompt
